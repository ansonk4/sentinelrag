...
def load_hf(object_class, model_name):
    try:
        obj = object_class.from_pretrained(model_name, local_files_only=True, force_download=False)
    except:
        # Force download if local cache is corrupted
        obj = object_class.from_pretrained(model_name, local_files_only=False, force_download=True)
    return obj
